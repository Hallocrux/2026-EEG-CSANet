...
from model.CSANet_HGD import CSANet
...
def main(config):
    ...
    for subject_id in range(1, 24):
        ...
        out_path = os.path.join(out_folder, config['network'], f'sub{subject_id}', random_folder)
        ...
        for fold, (train_index, test_index) in enumerate(tqdm(skf.split(eeg_data, labels),
                                                              total=n_splits,
            ...
            model = baseModel(net, config, optimizer, loss_func, result_savepath=fold_path)
