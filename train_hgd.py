...
def main(config):
    ...
    for subId in range(1, 15):
        ...
        out_path = os.path.join(
            out_folder, config["network"], "sub" + str(subId), random_folder
        )
        ...
        train_data, train_labels = load_HGD_data(
            os.path.join(data_path, "train"), data_file, label_file
        )
        train_labels = train_labels - 1
        test_data, test_labels = load_HGD_data(
            os.path.join(data_path, "test"), data_file, label_file
        )
...
if __name__ == "__main__":
    configFile = "config/csanet_hgd.yaml"
