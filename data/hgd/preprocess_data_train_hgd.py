...
data_path = r'D:/high-gamma-dataset/data/train'
save_path = r'process_data/train'
os.makedirs(save_path, exist_ok=True)

time_window = [0, 4000]  # epoch ms
C_sensors = ['FC5', 'FC1', 'FC2', 'FC6', 'C3', 'C4', 'CP5',
             'CP1', 'CP2', 'CP6', 'FC3', 'FCz', 'FC4', 'C5', 'C1', 'C2',
             'C6', 'CP3', 'CPz', 'CP4', 'FFC5h', 'FFC3h', 'FFC4h', 'FFC6h',
             'FCC5h', 'FCC3h', 'FCC4h', 'FCC6h', 'CCP5h', 'CCP3h', 'CCP4h', 'CCP6h',
             'CPP5h', 'CPP3h', 'CPP4h', 'CPP6h', 'FFC1h', 'FFC2h', 'FCC1h', 'FCC2h',
...
for sub_id in range(1, 15):
    ...
    def highpass_eeg_only(data):
        new_data = data.copy()
        new_data[:len(C_sensors), :] = highpass_cnt(data[:len(C_sensors), :], 0, eeg_raw.info['sfreq'], filt_order=3,
                                                    axis=1)
        return new_data
    ...
    def standardize_eeg_only(data):
        ...
        new_data[:len(C_sensors), :] = exponential_running_standardize(
            data[:len(C_sensors), :].T, factor_new=1e-3, init_block_size=1000, eps=1e-4).T
        return new_data
    ...
    train_data, train_label = create_data_label_from_raw_mne(eeg_raw, marker_def, time_window)
