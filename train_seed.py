...
def main(config):
    ...
    for subId in range(1, 16):
        for iii in range(5):
            ...
            train_data, train_labels, test_data, test_labels = load_SEED_5_fold(
                data_path, subId, iii
            )
            ...
            print(
                "Trainable Parameters in the network are: " + str(count_parameters(net))
            )
            ...
            model = baseModel(
                net, config, optimizer, loss_func, result_savepath=out_path
            )
